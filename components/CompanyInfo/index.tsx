      ...
      <Container>
        <div className="grid gap-5 grid-cols-1 md:grid-cols-3">
          {companyInfo && companyInfo.map((info) => (
            <div key={info?.title} className="flex flex-col items-center bg-[#eff2f7] p-10 gap-8 rounded-md">
              {info?.icon === 'map' && <MapPin size={45} strokeWidth={1} color="#0c5adb" />}
              {info?.icon === 'clock' && <Clock3 size={45} strokeWidth={1} color="#0c5adb" />}
              {info?.icon === 'phone' && <PhoneCall size={45} strokeWidth={1} color="#0c5adb" />}
