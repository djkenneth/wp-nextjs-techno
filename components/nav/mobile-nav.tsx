...
export function MobileNav() {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="left" className="px-0 w-full" isClose={true}>
        <div className="flex justify-between items-center py-3 px-4 bg-primaryBlue">
          <MobileLink
            href="/"
            className="flex items-center"
            onOpenChange={setOpen}
          >
            <span className="text-white font-bold">TECHNO</span>
          </MobileLink>
          <Button size="icon" className="bg-transparent" onClick={() => setOpen(false)}>
            <X size={28} strokeWidth={2.75} className="text-white" />
          </Button>
        </div>
